...
const ProductoAgri = require('../models/Producto_Agri')
...
exports.asignarProducto = async(req, res, next) => {
    const proid = req.params.cond_numero;
    const asignarProductoAgri = await Productos.findAll({
        where: {
            cond_numero: proid
        }
    });
    const agricultores = await Agricultor.findAll();
    res.render('asignacionProducto', {
        nombrePagina : 'Asignar Producto',
        asignarProductoAgri,
        agricultores
    })
    console.log(asignarProductoAgri);
}

exports.nuevaAsignacion = async(req, res, next) => {
    const idpro = req.body.idPro;
    console.log('*', idpro);
    const idagro = req.body.idagro;
    console.log('**', idagro);

    try{
        await ProductoAgri.create({
            agricultor_cond_numero: idPro,
            producto_cond_nombre: idagro,
        });
        console.log("Producto asignado correctamente");
        res.redirect('/productos');
    }
    catch(err){
        console.log("Error al intentar Asignar producto");
        res.redirect('/productos');
    }
}